...
# Convert fee_rate to Decimal once at module load to avoid repeated coercions
_FEE_RATE: Decimal = Decimal(str(settings.payment.fee_rate))
_TWO_PLACES: Decimal = Decimal("0.01")


def _to_decimal(value) -> Decimal:
    """Safely coerce a numeric value to Decimal via string to avoid float repr issues."""
...
class ReconciliationService:
    ...
    All balances are Decimal to prevent IEEE 754 floating-point drift.
    """
    ...
    def record_transaction(self, tx: Transaction):
        ...
        All arithmetic is performed in Decimal with explicit ROUND_HALF_UP rounding
        to eliminate IEEE 754 accumulation errors.
        """
        amount: Decimal = _to_decimal(tx.amount)

        # Fee rounded to 2dp immediately — no intermediate float drift
        fee: Decimal = _round2(amount * _FEE_RATE)
        net: Decimal = _round2(amount - fee)

        # Both ledger and actual now use exact Decimal arithmetic
        ...
        # Persist Decimal values back to transaction model
        # Convert to float only at the boundary if the model requires it;
        # prefer storing as Decimal or str in the model for full correctness.
        tx.fee = float(fee)
        tx.net_amount = float(net)

        # Drift should now be effectively zero; compute and emit for observability
        ...
        drift_threshold = _to_decimal(str(cfg.drift_threshold))
        if drift > drift_threshold:
            ...
            m.app_errors_total.labels(
                component="reconciliation", error_type="decimal_drift"
            ).inc()
    ...
    def get_summary(self) -> dict:
        drift: Decimal = abs(self._ledger_balance - self._actual_balance)
        actual_balance_f = float(self._actual_balance)
        return {
            "ledger_balance": float(self._ledger_balance),
            "actual_balance": actual_balance_f,
            "drift": float(drift),
            "drift_pct": float((drift / Decimal(str(max(actual_balance_f, 0.01)))) * 100),
            "transaction_count": self._transaction_count,
            "total_fees_ledger": float(self._fee_ledger),
            "total_fees_actual": float(self._fee_actual),
            "status": (
                "critical" if drift > _to_decimal(str(cfg.drift_threshold)) * 10
                else "mismatch" if drift > _to_decimal(str(cfg.drift_threshold))
                else "ok"
            ),
        }
