...
Fixes applied:
  - BUG 1: Per-key asyncio.Lock makes idempotency check-and-set atomic.
  - BUG 2: Both DB phases run inside one connection transaction; compensating
            credit is issued on failure (saga rollback).
  - BUG 3: process_refund now acquires _acct_lock then _order_lock (same
            order as process_payment) to eliminate the deadlock scenario.
"""
import asyncio
import json
import logging
...
# ---------------------------------------------------------------------------
# Idempotency store — guarded by per-key locks so check+write is atomic.
# ---------------------------------------------------------------------------
_idempotency_store: Dict[str, Transaction] = {}

# Registry of per-key locks.  A lock is created on first use and kept until
# the key is committed to _idempotency_store (at which point the lock is
# still retained so late arrivals get the cached result immediately).
_idempotency_locks: Dict[str, asyncio.Lock] = {}
_idempotency_locks_meta_lock = asyncio.Lock()   # guards _idempotency_locks
...
# ---------------------------------------------------------------------------
# Global ordering locks — BOTH code paths acquire in the same order:
...
class PaymentProcessor:
    ...
    async def process_payment(self, tx: Transaction) -> Transaction:
        ...
        Idempotency is now enforced atomically:
          - Acquire per-key lock before reading or writing _idempotency_store.
          - Any concurrent request with the same key blocks on the lock and
            receives the cached result once the first request completes.

        DB writes are wrapped in a single connection-level transaction:
          - On failure after phase-1 debit, a compensating credit is issued
            within the same transaction before rolling back, so the account
            balance is never left in an inconsistent state.
        """
        start = time.monotonic()
        m.active_payment_requests.inc()

        # Obtain (or create) the per-key lock BEFORE touching the store.
        ...
        try:
            ...
            # Only one coroutine per key can be inside this block at a time.
            async with key_lock:
                ...
                # while we are processing sees PROCESSING status and waits for
                # the lock rather than racing through.
                #
                # We store a sentinel (the in-flight tx object) so the slot is
                # occupied.  It will be overwritten with the final result below.
            ...
            # ── Step 3: Acquire global locks (consistent order) ───────────────
            try:
            ...
            # ── Step 4: Single atomic DB transaction (both phases) ────────────
            conn = None
            try:
                conn = db_pool.acquire()

                # Begin explicit transaction so both writes are atomic.
                conn.execute("BEGIN")
                ...
                # Roll back the entire transaction — neither phase is committed.
                # The account balance is left unchanged; no orphaned debit.
                if conn:
                    try:
                ...
                # Remove the in-flight sentinel so callers may retry.
                async with key_lock:
                    if _idempotency_store.get(tx.idempotency_key) is tx:
                        del _idempotency_store[tx.idempotency_key]
                        m.idempotency_cache_size.set(len(_idempotency_store))
            ...
            # Overwrite sentinel with the completed transaction.
            async with key_lock:
                _idempotency_store[tx.idempotency_key] = tx
                m.idempotency_cache_size.set(len(_idempotency_store))
    ...
    async def process_refund(
        ...
        Lock acquisition order is now identical to process_payment:
          _acct_lock  →  _order_lock
        This eliminates the deadlock that occurred when process_refund
        previously acquired _order_lock first.
        """
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"[Refund] Rollback failed: {rb_exc}")
